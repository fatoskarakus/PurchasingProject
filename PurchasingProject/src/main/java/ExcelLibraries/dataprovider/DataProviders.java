 ...
 * 
 */
package ExcelLibraries.dataprovider;
...
public class DataProviders {
	...
	ExcelLibraries obj = new ExcelLibraries();
	...
	public Object[][] getCredentials() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("Credentials", "Username", i + 2);
			}
		}
		return data;
	}
...
//Class --> AccountCreationPage  Test Case--> verifyCreateAccountPageTest	
	...
	public Object[][] getEmail() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("Email", "Email", i + 2);
			}
		}
		return data;
	}
...
//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
	...
	public Object[][] getProduct() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("ProductDetails", "Product", i + 2);
			}
		}
		return data;
	}
	...
	public Object[][] getProductnName() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("SearchProduct", "Product", i + 2);
			}
		}
		return data;
	}
	...
	public Object[][] accountCreation() {
		...
		for (int i = 0; i < actRows; i++) {
			...
			for (int j = 0; j < column; j++) {
				hashMap.put(obj.getCellData("AccountCreationData", "Email", 1),
						obj.getCellData("AccountCreationData", "Email", i + 2));
			}
			data[i][0]=hashMap;
		}
		return data;
